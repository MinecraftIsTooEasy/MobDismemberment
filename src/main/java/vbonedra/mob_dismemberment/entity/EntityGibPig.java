...
public class EntityGibPig extends EntityGibBase {
    public EntityGibPig(World world) {
        super(world);
    }

    public EntityGibPig(World world, EntityLivingBase gibParent, int gibType, Entity explosion_source) {
        ...
        // gibType: 0 = Head, 1 = Body, 2 = Back Left Leg, 3 = Back Right Leg, 4 = Front Left Leg, 5 = Front Right Leg
        if (gibType == 0) {
            this.setSize(0.5F, 0.5F);
            this.rotationYaw = gibParent.rotationYaw;
            startX += 0.375D * Math.sin(mathYaw);
            startZ -= 0.375D * Math.cos(mathYaw);
            startY += 0.75D;
        } else if (gibType == 1) {
